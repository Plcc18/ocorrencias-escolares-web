...
export function PageHeader({ title, description, children, className }: PageHeaderProps) {
  return (
    <div
      className={cn(
        'h-16 flex items-center justify-between pl-12 md:px-6 border-b border-sidebar-border bg-sidebar text-sidebar-foreground shrink-0',
        className,
      )}
    >
      <div>
        <h2 className="text-base font-semibold text-sidebar-foreground leading-none">{title}</h2>
        {description && <p className="text-sm text-sidebar-foreground/65 mt-0.5">{description}</p>}
      </div>
      {children && (
