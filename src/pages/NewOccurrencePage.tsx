...
export default function NewOccurrencePage() {
  ...
  const selectedTeacher = isAdmin ? teachers?.find((t) => t.id === selectedTeacherId) : null;
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center gap-2 mb-4">
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                  gradeId ? 'bg-green-500 text-white' : 'bg-muted text-muted-foreground'
                }`}
              >
          ...
          <div
            className={`bg-card border border-border rounded-xl p-5 transition-opacity ${
              !gradeId ? 'opacity-40 pointer-events-none' : ''
            }`}
          >
            <div className="flex items-center gap-2 mb-4">
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                  studentId ? 'bg-green-500 text-white' : 'bg-muted text-muted-foreground'
                }`}
              >
                    ...
                    filteredStudents.map((s) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-sm hover:bg-muted transition-colors flex items-center justify-between ${
                          studentId === s.id ? 'bg-primary/5 font-medium' : ''
                        }`}
                      >
                        <div>
          ...
          {isAdmin ? (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                    selectedTeacherId ? 'bg-green-500 text-white' : 'bg-muted text-muted-foreground'
                  }`}
                >
                  {selectedTeacherId ? (
                    <CheckCircle2 className="size-4" />
                  ) : (
                    String(teacherStepNumber)
                  )}
                </div>
              ...
              <CustomSelect
                ...
                onChange={(e) =>
                  setSelectedTeacherId(e.target.value ? Number(e.target.value) : null)
                }
                className="w-full"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    A ocorrência será registrada em seu nome:{' '}
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                  occurrenceType ? 'bg-green-500 text-white' : 'bg-muted text-muted-foreground'
                }`}
              >
          ...
          <div
            className={`bg-card border border-border rounded-xl p-5 transition-opacity ${
              !occurrenceType ? 'opacity-40 pointer-events-none' : ''
            }`}
          >
            <div className="flex items-center gap-2 mb-4">
              <div
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 font-medium">{selectedGrade?.displayName}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">Tipo:</span>
                  <span className="ml-2">
                    {occurrenceType && <OccurrenceBadge type={occurrenceType} />}
                  </span>
                </div>
                <div>
